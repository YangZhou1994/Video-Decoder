...
#include "vpe_avio_mem_context.h"
#include "vpe_decoder.h"

#include <unistd.h>

bool vpeDecoder(int buffer_len, char* buffer, VideoData& video_frames)
{
	...
	VpeAvioMemContext avio_ctx(buffer, buffer_len);
		...
		if (packet->stream_index == videoindex)
		{
			...
			if (frameFinished)
			{
				sws_scale(pSWSCtx, pFrame->data, pFrame->linesize, 0,
						...
						pFrameRGB->linesize);
				char* frame_temp = new char[pFrame->width * pFrame->height * 3];
				memcpy(frame_temp, pFrameRGB->data[0],
						pFrame->width * pFrame->height * 3);
				video_frames.frames.push_back(frame_temp);
				video_frames.width = pFrame->width;
				video_frames.height = pFrame->height;
				printf("  ==== i: %d\n", i);
				i++;
			}
		}
